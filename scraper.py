...
def scrape_funda_detail(url):
    ...
    size_elem = soup.select_one(
        "li.object-kenmerken__item--surface-area span.object-kenmerken__value"
    )
    size = size_elem.get_text(strip=True) if size_elem else "N/A"

    # Extract number of rooms - example selector, adjust as needed
    rooms_elem = soup.select_one(
        "li.object-kenmerken__item--number-of-rooms span.object-kenmerken__value"
    )
